# Architekt — documentation index

Internal documentation for **Architekt Factory Platform** (ede-bzh/architekt-factory-platform).

## Operations & delivery

| Document | Purpose |
|----------|---------|
| [LOCAL-DEV.md](LOCAL-DEV.md) | Run the platform locally (`PLATFORM_LLM_PROVIDER=demo`, ports, CLI) |
| [RELEASE.md](RELEASE.md) | Versioning (`platform/VERSION`), tags `v*`, CI gates, rollback |
| [PLATFORM-BACKLOG.md](PLATFORM-BACKLOG.md) | Doc rebrand waves, wiki/README gates, completeness checklist |
| [REBRAND-DOC-AUDIT.md](REBRAND-DOC-AUDIT.md) | Wave 8 audit: P0–P3 scope, forbidden-string gates, file inventory |

## Product roadmap (phases)

| Phase | Document |
|-------|----------|
| 0 — Rebrand | [phase-0-rebrand.md](phase-0-rebrand.md) |
| 1 — Offer catalog | [phase-1-offer-catalog.md](phase-1-offer-catalog.md) |
| 2 — Practices | [phase-2-practices.md](phase-2-practices.md) |
| 3 — Pilot | [phase-3-pilot.md](phase-3-pilot.md) |
| 4 — First clients | [phase-4-first-clients.md](phase-4-first-clients.md) |
| 5 — IDP | [phase-5-idp.md](phase-5-idp.md) |
| 6 — Client portal | [phase-6-client-portal.md](phase-6-client-portal.md) |
| 7 — SaaS | [phase-7-saas.md](phase-7-saas.md) |

## Related

- Root agent context: [CLAUDE.md](../../CLAUDE.md)
- Rebrand ADR: [docs/adr/001-rebrand-architekt.md](../adr/001-rebrand-architekt.md)
- User wiki (EN/FR): [docs/wiki/Home.md](../wiki/Home.md), [docs/wiki/Home‐FR.md](../wiki/Home‐FR.md)
