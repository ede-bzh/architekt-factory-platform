...
| `ARCHITEKT_API_KEY` | **Alias (6 months)** | Accepted until alias sunset; same value as `ARCHITEKT_API_KEY` |
...
Legacy deployments may still export `ARCHITEKT_API_KEY`; the middleware treats both names as equivalent during the 6-month alias window.
...
The `sf` CLI uses the same Bearer token (from `ARCHITEKT_API_KEY` or legacy `ARCHITEKT_API_KEY`):
