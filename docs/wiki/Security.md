...
`AuthMiddleware` (`platform/security.py`) validates Bearer tokens with SHA-256 comparison. Mutations (`POST`, `PATCH`, `DELETE` on `/api/*`) require a valid key when `ARCHITEKT_API_KEY` is set.
