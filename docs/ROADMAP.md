...
> **Mise à jour** : 2026-05-26 — vague E livrée dépôt (PR #27, branche `cursor/wave-e-full-replace-7576`)
...
| **Plateforme waves 0–8** | Livré dont vague E dépôt (runtime `architekt_platform`) | Rebuild Azure post-merge, mutation 50 % global |
...
### Vague E ✅ (livré dépôt — PR #27)

- [x] `platform/runtime.py` + image Docker `architekt_platform` (sans alias `MACARON_*` ni symlink `macaron_platform` — ADR-001)
- [x] Runbook [`architekt/WAVE-E-RUNBOOK.md`](architekt/WAVE-E-RUNBOOK.md) + Helm `deploy/helm/architekt/`
- [x] Wiki Deployment / API / Sécurité EN/FR — vérité unique `ARCHITEKT_*` et `/opt/architekt`
- [x] Remplacement intégral marque Macaron → Architekt (code, CI, CLI, métriques `architekt_*`, chart Helm)
- [x] Gates & tests vague E (`test_wave_e_runtime`, `test_doc_no_macaron_user_facing`, branding, CSP)
- [ ] Rebuild prod Azure + validation OTEL / hotpatch CI (ops — voir runbook)
...
### Vague E ops (post-merge PR #27)

- [ ] Rebuild image + compose sur `/opt/architekt` (OTEL `architekt-platform`, hotpatch CI)
